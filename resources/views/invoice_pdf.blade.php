        ...
        <table class="table w-100 mt-10">
            ...
            <tr>
                ...
                <td>
                    <div class="box-text">
                        <p>{{$userData->name}}</p>
                        <p>{{$userData->flat_no}} {{$userData->apartment_name}} {{$userData->area}}
                            {{$userData->landmark}}
                        </p>
                        <p>{{$userData->city}} {{$userData->pincode}}</p>
                        <p>Contact: {{$userData->s_phone}}</p>
                    </div>
                </td>
            </tr>
        </table>
    ...
    <div class="table-section bill-tbl w-100 mt-10">
        <table class="table w-100 mt-10">
            <tr>
                ...
                <th class="w-50 rupee-symbol">MRP per qty(₹)</th>
                <th class="w-50 rupee-symbol">MRP(₹)</th>
                <th class="w-50 rupee-symbol">Discount(₹)</th>
                <th class="w-50 rupee-symbol">Price(₹)</th>
                <th class="w-50 rupee-symbol">Tax(₹)</th>
                <th class="w-50 rupee-symbol">Total(₹)</th>

            </tr>
            @foreach ($cartProducts as $product)
                <tr align="center">
                    <td class="malayalam-text">{{ $product['title'] ?? 'N/A' }} - {{ $product['qty_text'] ?? 'N/A' }}</td>
                    <td>{{ $product['qty'] ?? 'N/A' }}</td>
                    <td>{{ number_format($product['mrp'] ?? 0, 2) }}</td> <!-- Format MRP -->
                    <td>{{ number_format($product['totalAmount'] ?? 0, 2) }}</td> <!-- Format MRP -->
                    <td>{{ number_format($product['discount'] ?? 0, 2) }}</td> <!-- Format MRP -->
                    <td>{{ number_format($product['totalPrice'] ?? 0, 2) }}</td> <!-- Format Price -->
                    <td>{{ number_format($product['totalTax'] ?? 0, 2) }}</td> <!-- Format Tax -->
                    <td>{{ number_format($product['netAmount'] ?? 0, 2) }}</td> <!-- Format Total Price -->
                </tr>
            @endforeach
            <tr>
                <td colspan="8">
                    <div class="total-part">
                        ...
                        <div class="total-right w-25 float-left text-bold" align="right">
                            ...
                            <p class="rupee-symbol">
                                {{ '₹' . number_format($order->order_amount - (int) $order->delivery_charge, 2) }}
                            </p>
                            <!-- {{ '₹' . number_format($order->price + $taxPrice, 2) }} -->
                            <!-- Delivery Charge -->
